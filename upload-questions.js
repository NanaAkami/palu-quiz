#!/usr/bin/env node
/**
 * 上传题目到 Firebase Realtime Database
 * 使用方法: node upload-questions.js
 */
...
const FIREBASE_CONFIG = {
    apiKey: 'AIzaSyB8s63wS3n7hB3lW_rH3EXAMPLE_ONLY',
    databaseURL: 'https://nana01-9a9ee-default-rtdb.asia-southeast1.firebasedatabase.app'
};
...
// Firebase REST API 函数
async function firebasePut(path, data) {
    return new Promise((resolve, reject) => {
        const jsonData = JSON.stringify(data);
        const options = {
            hostname: 'nana01-9a9ee-default-rtdb.asia-southeast1.firebasedatabase.app',
            path: path + '.json',
            method: 'PUT',
            headers: {
...
async function uploadQuestions() {
    console.log('🚀 开始上传题目到 Firebase...\n');
    
    try {
        // 1. 上传题目（不含答案）
        console.log('📤 上传题目数据...');
        await firebasePut('/questions', questionsData);
        console.log(`  ✅ 已上传 ${Object.keys(questionsData).length} 道题目\n`);
        
        // 2. 上传答案（单独路径，需要认证才能读取）
        console.log('📤 上传答案数据...');
        await firebasePut('/answers', answersData);
        console.log(`  ✅ 已上传 ${Object.keys(answersData).length} 个答案\n`);
        
        // 3. 上传配置
        console.log('📤 上传配置数据...');
        await firebasePut('/config', {
        ...
        console.log('\n下一步：');
        console.log('1. 设置 Firebase Database 安全规则');
        console.log('2. 部署前端到 GitHub Pages');
